...
const BooksSingleCard = ({book}) => {
    const [showModel,setShowModek] = useState(false)
  return (
    ...
    <div className='flex justify-start items-center gap-x-2 mt-4 p-4'>
        <BiShow className='text-3xl text-blue-800 hover:text-black cursor-pointer' onClick={()=>setShowModek(true)}/>
    ...
    {
        showModel && (
            <BookModel book={book} onClose={()=>setShowModek(false)}/>
        )
    }
</div>
  )
